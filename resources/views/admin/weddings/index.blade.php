    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <h2 class="font-serif text-2xl font-semibold text-red-900 leading-tight">
                {{ __('Wedding Management') }}
            </h2>
            <a href="{{ route('weddings.create') }}" class="inline-flex items-center px-4 py-2 bg-red-900 text-white rounded-xl hover:bg-red-800 transition shadow-sm">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-stone-200">
                    ...
                    <p class="text-3xl font-bold text-red-900 mt-1">{{ $totalWeddings ?? 0 }}</p>
                </div>
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-stone-200">
                    <p class="text-sm text-stone-500 uppercase tracking-wider">Published</p>
                    <p class="text-3xl font-bold text-amber-700 mt-1">{{ $publishedWeddings ?? 0 }}</p>
                </div>
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-stone-200">
                    <p class="text-sm text-stone-500 uppercase tracking-wider">Total Guests</p>
                    <p class="text-3xl font-bold text-green-700 mt-1">{{ $totalGuests ?? 0 }}</p>
                </div>
            </div>
                        ...
                        <tbody class="bg-white divide-y divide-stone-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-stone-500">{{ $wedding->slug }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-stone-700">
                                    {{ \Carbon\Carbon::parse($wedding->event_date)->format('d M Y') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-stone-700">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-stone-500">
                                    ...
                                    <a href="{{ route('weddings.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-red-900 text-white rounded-xl hover:bg-red-800 transition">
                                        Create Wedding
                                    </a>
                                </td>
                            </tr>
