<section id="rsvp" class="py-16 sm:py-20 px-6 bg-white section-fade">
    <div class="max-w-lg mx-auto">
        ...
        {{-- ✅ SUCCESS STATE: Gunakan x-show (Alpine), BUKAN @if (Blade) --}}
        <div x-show="rsvpSubmitted"
            ...
            {{-- Optional: Tombol untuk kirim RSVP lain --}}
            {{-- <button @click="rsvpSubmitted = false"
                    class="mt-4 text-sm text-green-700 hover:text-green-900 underline">
                Kirim RSVP Lain
            </button> --}}
        </div>

        {{-- ✅ RSVP FORM: Juga gunakan x-show dengan negasi --}}
        <div x-show="!rsvpSubmitted"
             x-transition:enter="transition ease-out duration-300"
             x-transition:enter-start="opacity-0 transform scale-95"
             x-transition:enter-end="opacity-100 transform scale-100">
            ...
            <form @submit.prevent="submitRSVP()" class="bg-gradient-to-br from-stone-50 to-amber-50/30 rounded-2xl p-6 shadow-lg space-y-5">
                ...
                {{-- ✅ TAMBAHKAN INI: Hidden input untuk unique_code --}}
                @php
