...
<body class="bg-stone-50 text-stone-800 font-sans antialiased">
    <div class="min-h-screen flex flex-col items-center justify-center p-6 text-center">
        ...
        <p class="text-lg text-stone-600 max-w-xl mb-8">Platform undangan pernikahan digital yang elegan, sakral, dan
            modern untuk momen terindah Anda.</p>

        <div class="flex gap-4">
            @if (Route::has('login'))
            @auth
            <a href="{{ url('/dashboard') }}"
                class="px-6 py-3 bg-red-900 text-white rounded-xl font-medium shadow-lg hover:bg-red-800 transition">
                Dashboard
            </a>
            @else
            <a href="{{ route('login') }}"
            ...
            <a href="{{ route('register') }}"
                class="px-6 py-3 bg-white text-red-900 border-2 border-red-900 rounded-xl font-medium hover:bg-red-50 transition">
                Register
            </a>
            @endif
            @endauth
